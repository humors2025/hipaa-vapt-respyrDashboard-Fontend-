...
export function SalesPagination({ page, limit, total, totalPages, onPageChange, disabled }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 flex-wrap">
      <span className="text-[#A1A1A1] text-[12px]">
        Showing {formatCount(from)}–{formatCount(to)} of {formatCount(total)} purchases
      </span>
      <nav aria-label="Purchase pages" className="flex items-center gap-1 flex-wrap">
        <button
          type="button"
...
export default function PurchaseTable({ rows, loading, updating, error, onRetry, onSelect, emptyMessage }) {
  ...
  return (
    <div className="relative overflow-x-auto rounded-[10px] border border-[#E1E6ED]">
      <table className={`w-full text-[12px] min-w-[1100px] transition-opacity ${updating ? "opacity-50" : "opacity-100"}`} aria-busy={updating}>
        <thead>
          ...
          {rows.map((r, i) => {
            ...
            return (
              ...
              >
                ...
                <td className="py-2.5 px-2.5 text-[#535359] whitespace-nowrap">{r.plan?.name || r.plan?.code || "—"}</td>
                <td className="py-2.5 px-2.5">
                  <SourceBadge source={r.purchase_source} />
                </td>
                <td className="py-2.5 px-2.5 text-[#535359] whitespace-nowrap">
                  {r.purchase_source === "trainer_code" && r.trainer?.name ? r.trainer.name : <Dash />}
                </td>
