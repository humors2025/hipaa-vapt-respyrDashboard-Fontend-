...
import { validateHumanName } from "@/utils/inputValidation";
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              ...
              <textarea
                value={reason}
                onChange={(e) =>
                  setReason(e.target.value.replace(/[<>]/g, "").slice(0, 255))
                }
...
function InviteForm({ partnerCode, onInviteSent }) {
  ...
  const onSubmit = async (e) => {
    ...
    const nameResult = validateHumanName(name, "Client name", {
      minLength: 2,
      maxLength: 100,
    });
    if (!nameResult.ok) {
    ...
    try {
      ...
      toast.success(`Invite sent to ${nameResult.value} — ${clientEmail}`);
      reset();
      onInviteSent();
    } catch (err) {
      const msg = err?.data?.message || err?.message || "";
      if (msg.toLowerCase().includes("already has a pending invitation")) {
        toast.success(`Invite sent to ${nameResult.value}`);
        reset();
        onInviteSent();
      } else {
